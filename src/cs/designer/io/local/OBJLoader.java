...
public class OBJLoader extends LoaderBase implements Loader {
    ...
    private static final String JAVA_3D_MATERIALS =
            "newmtl amber\n" +
                    ...
                    "Ns 60.0000\n" +
                    "sharpness 60.0000\n";
    private final static Map<String, AppearanceMeta> DEFAULT_APPEARANCES;
    ...
    private Map<String, AppearanceMeta> appearances;
    ...
    private Scene parseObjectStream(Reader reader,
        ...
        this.appearances = new HashMap<String, AppearanceMeta>(DEFAULT_APPEARANCES);
    ...
    private SceneBase createScene() {
        ...
        final BranchGroup sceneRoot = new BranchGroup();
        ...
        for (Group group : this.groups.values()) {
            ...
            if (geometries != null
                ...
                while (i < geometries.size()) {
                    ...
                    AppearanceMeta appearance = getAppearance(firstGeometryMaterial);
                    ...
                    if (appearance != null) {
                        appearance = (AppearanceMeta) appearance.cloneNodeComponent(false);
                    ...
                    final Shape3DMeta metaShape3D = new Shape3DMeta(geometryArray, appearance);
                    sceneRoot.addChild(metaShape3D);
                    scene.addNamedObject(group.getName() + (i == 0 ? "" : String.valueOf(i)), metaShape3D);
     ...
     */
    private AppearanceMeta getAppearance(String material) {
        AppearanceMeta appearance = null;
        if (material != null) {
     ...
     */
    private static Map<String, AppearanceMeta> parseMaterialStream(Reader reader,
                                                                   URL baseUrl) throws IOException {
        Map<String, AppearanceMeta> appearances = new HashMap<String, AppearanceMeta>();
        AppearanceMeta currentAppearance = null;
        StreamTokenizer tokenizer = createTokenizer(reader);
        while (tokenizer.nextToken() != StreamTokenizer.TT_EOF) {
            switch (tokenizer.ttype) {
                case StreamTokenizer.TT_WORD:
     ...
     */
    private static AppearanceMeta parseMaterialLine(StreamTokenizer tokenizer,
                                                    Map<String, AppearanceMeta> appearances,
                                                    AppearanceMeta currentAppearance,
                                                    URL baseUrl) throws IOException {
        if ("newmtl".equals(tokenizer.sval)) {
            // Read material name newmtl name
            if (tokenizer.nextToken() == StreamTokenizer.TT_WORD) {
                currentAppearance = new AppearanceMeta();
            ...
            Color3f diffuseColor = new Color3f(parseNumber(tokenizer),
                    parseNumber(tokenizer), parseNumber(tokenizer));
            if (currentAppearance != null) {
                MaterialMeta material = getMaterial(currentAppearance);
            ...
            if (imageFileName != null) {
                ...
                if (textureImage != null) {
                    currentAppearance.setTextureImage(textureImage);
                }
            } else {
     ...
     */
    private static MaterialMeta getMaterial(AppearanceMeta appearance) {
         MaterialMeta material =  new MaterialMeta(appearance.getMaterial());
        if (material == null) {
            material = new MaterialMeta();
